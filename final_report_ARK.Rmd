...
For the second part of the project, we chose to work on the data available on Health in the original data set we found on the specified official government website. The health part of the data had information from US Census 2012-2017 for obesity and availability of recreational facilities and 2008-2013 for diabetes from all the counties of the USA. To simplify the data exploration and analysis process, the mean was taken for the county data to represent their respective state. The data of 3143 observations was shortened to 51 observations and then used for further processing. 

There were several questions that came to mind during exploration. Starting with what the general trend of Diabetes over the years 2008-2013 looks like? Was US able to control the increasing obesity problem? Ideally, if more recreational facilities are available to the public, they might be using these facilities more often which then might help reduce obesity or other health problems. So, I wanted to look at how reality is over the ideal condition. Whether there is a negative correlation between the recreational facilities available per thousand people and the % change in obesity as there ideally should be if more people use such facilities. The last one was to compare the East Coast and the West Coast region states to see if one region is doing better than the other? 
...
The dataset 'Food Environment Atlas' was downloaded from this [site](https://www.ers.usda.gov/data-products/food-environment-atlas/data-access-and-documentation-downloads/). Furthermore, different categories of interest in this dataset was divided among team members to analyze.
The data was available for all the counties in USA and it was observed that in majority of the states, all the different counties had same value of observations for the diabetes, obesity and recreation facilities. This is why for the creation of plots to explore Diabetes & Obesity trends, Comparison of %Change in recreational facilities and %Change in obesity, mean of all the observations of the counties of a state was used to represent the respective state. After this, obesity and recreation facilities data was also separated according to east coast and west coast regions. This region was added into another column which was created using 'mutate' function of 'dplyr' package.

To construct our maps, we made use of the packages ggplot2 and sf. The ggplot2 package allowed for the different spatial scales to be easily overlaid and plotted with other helpful addition such as title, scale, and color of scale to be easily adjusted. To actually read in and map the geometry of the data we used the sf package, which stands for spatial features. The geometry for states and counties was gathered from the US Census Bureau tiger-line files and read in with the sf package. The data for the urban areas was gathered from Data.gov, which complies data sets from the US Census Bureau for other uses and interpretations. 
We used the package readxl in order to read in data stored in excel spreadsheets and turn it into dataframes that could be utilized in R programming. The package plyr was used in order to reorganize the data into select dataframes for easier comparison and analysis. The tidyverse package was used in order to clean up the data for better analysis.  
...
Most interesting exploration I was able to do was to explore the %Change in Obesity in context to the %Change in recreational facilities available per thousand people. In original data set obesity data was available for 2012 and 2017. So %Change from 2012 to 2017 was calculated in a different column so that it can be plotted with %Change in recreational facilities available per thousand people with the states on X-axis.

In this plot, more red dots in the bottom part shows that in majority of the states the number of recreational facilities available actually decreased, and this was seen with greater %change in obesity in majority. Highlights are the states like California, Washington D.C., North Hampshire which got higher %change in recreational facilities and seems to be doing good at controlling the %change in obesity over 2012-2017.
...
When comparing the data collected on the rates of food insecurity, percentage of state populations experiencing low food access, the number of grocery stores available to local populations, and state population levels, it seems there is a clear correlation if not relationship between these factors. When local populations have fewer grocery stores available, this means they have less access to healthy food. This access is reflected in the high rates of low food access and food insecurity. 

From the analysis of %Change in obesity (2012-2017) and %Change in recreational facilities available per thousand people in the same time period it was observed in plot that the states having high increase in Recreation facilities available to public had less increase in %change in obesity. This was also supported by the correlation factor calculated that came out to be negative as expected.

Based on the data that was represented in the maps, there does appear to be a correlation between percent of the population with low access to food being in regions that are more rural and away from the urban centers. Also, when comparing obesity to food access, the areas with a higher rate of obesity were more directed to the eastern part of the country, which matched the areas showing a lower percentage of low access to food. Therefore, the east has an easier access to food and which is possibly contributing to their higher obesity.


In conclusion, the Food Atlas data set is very beneficial in analyzing food accessibility and health trends in the United States. There is a wide range of variables that the Food Atlas covers and we only scratched the surface on. Exploring the data can be helpful to community leaders in order to understand and mitigate low access to food through food programs at school or for the community and number of stores. They may also draw conclusions on the amount of healthy food available and the health of the population to make positive changes for their future.

## References 
1) “Food Environment Atlas.” USDA ERS - Food Environment Atlas. U.S. Department of Agriculture. https://www.ers.usda.gov/data-products/food-environment-atlas/. 
2) US Census Bureau. Census.gov. https://www.census.gov/. 
3) Chobot, A., Górowska-Kowolik, K., Sokołowska, M., & Jarosz-Chobot, P. (2018). Obesity and diabetes-Not only a simple link between two epidemics. Diabetes/metabolism research and reviews, 34(7), e3042. https://doi.org/10.1002/dmrr.3042
