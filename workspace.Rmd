...
healthdata<-mutate(healthdata, region= case_when(
      State=="CA"~"West",
      State=="OR"~"West",
      State=="WA"~"West",
      State=="WV"~"East",
...
ew<-ggplot()+
  geom_point(data=eastwest,aes(x=State, y=RECCHG, colour="red"))+
  geom_point(data=eastwest,aes(x=State, y=PCTCHGRECPTH, colour="blue"))
eastwestplot<-ew+labs(title = "%Change in Obesity and Rec: Eastcoast V/S Westcoast", x = "Regions", y = "% Change", color = "Legend Title\n") +
  scale_color_manual(labels = c("RECCHG", "PCTCHGOBESITY"), values = c("red", "blue"))
eastwestplot
#eastwestplot + aes(x = fct_inorder(region))
...
p2+labs(title = "%Change in Total number of Recreation facilities V/S Obesity", x = "States", y = "% Change", color = "Legends\n") +
...
p3 <- ggplot()+
  geom_point(data=healthdata,aes(x=State, y=PCTCHGRECPTH, colour="red"))+
  geom_point(data=healthdata,aes(x=State, y=PCTCHGOBESITY, colour="blue"))
p4<-p3+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
p4+labs(title = "%Change in Recreation Facilities Per Thousand Population V/S Obesity", x = "States", y = "% Change", color = "Legends\n") +
...
health %>% slice_max(PCT_OBESE_ADULTS17) #West Virgina
